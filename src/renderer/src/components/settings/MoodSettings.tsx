        ...
        {moodTags.length === 0 ? (
          ...
          <ul className="flex flex-wrap gap-2 mb-4">
            {moodTags.map((tag) => (
              <li
                key={tag.id}
                className="group inline-flex items-center gap-1.5 rounded-full bg-paper border border-ink/15 pl-3 pr-1.5 py-1 font-serif text-ink text-sm"
              >
                <span>{tag.label}</span>
                <button
                  type="button"
                  ...
                  className="grid place-items-center w-5 h-5 rounded-full text-ink-soft hover:bg-ink/10 hover:text-rust transition-colors"
                >
